...
The framer appears to the host as an Ethernet interface.  The interface hardware address starts with aa-00-03 (an unused OUI once assigned to DEC and then rendered obsolete by IEEE 802 -- see `ether-i.pdf`).  The next 23 bits of the interface address are random, derived from the Raspberry Pico unique board ID, and the low order bit is 0 so the interface address is even.  The framer appears as an Ethernet device attached to that interface; its address is one larger than the interface address. 
